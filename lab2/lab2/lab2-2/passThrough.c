...
uint8_t* iv[16] = { 0x75, 0x52, 0x5f, 0x69, 0x6e, 0x74, 0x65, 0x72, 0x65, 0x73, 0x74, 0x69, 0x6e, 0x67, 0x21, 0x21 };
...
--*/
{
    ...
    status = FltGetFileNameInformation(
        ...
        &NameInfo);
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"mylab2");
    if (!NT_SUCCESS(status)) {
        ...
        if (
            ...
            if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
                ...
                DbgPrint("***Write.Length = %d", Data->Iopb->Parameters.Write.Length);
                DbgPrint("***Write.ReadBuffer %s", Data->Iopb->Parameters.Write.WriteBuffer);
                ...
                DbgPrint("***MajorFunction = %x", Data->Iopb->MajorFunction);
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        if (
            ...
            if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
                ...
                struct AES_ctx ctx;
                AES_init_ctx_iv(&ctx, key, iv);
                DbgPrint("***Read.Length = %d", Data->Iopb->Parameters.Read.Length);
                DbgPrint("***Read.ReadBuffer %s", Data->Iopb->Parameters.Read.ReadBuffer);

                AES_CBC_decrypt_buffer(&ctx, Data->Iopb->Parameters.Read.ReadBuffer, Data->Iopb->Parameters.Read.Length);
                DbgPrint("***Read.ReadBuffer %s", Data->Iopb->Parameters.Read.ReadBuffer);
