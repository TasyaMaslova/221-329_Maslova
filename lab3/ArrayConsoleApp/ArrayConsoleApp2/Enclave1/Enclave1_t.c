...
static sgx_status_t SGX_CDECL sgx_getDB(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_getDB_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_getDB_t* ms = SGX_CAST(ms_getDB_t*, pms);
	ms_getDB_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_getDB_t), ms, sizeof(ms_getDB_t))) {
	...
	getDB(_in_buffer, _tmp_len, __in_ms.ms_i);
	if (_in_buffer) {
